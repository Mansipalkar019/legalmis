function validate_add_invoice_type(ele){
    hide_message_box(ele);
    var hasError = 0;
    var servicename=$('#servicename').val();
     if(jQuery.trim(servicename) == '')
    {
        showError("Please Enter Invoice Type","servicename");hasError=1;
    ...
    if(hasError==1){
        return false;
            ...
            jQuery.ajax({
                dataType: 'json',
                type: "POST",
                url: bases_url+"Masters/addinvoicetype",
                data: {'jsonObj' : q},
                cache: false,
                beforeSend: function(){ 
                ...
                success: function(res){
                    jQuery(".btn-quirk").text('Submit').prop('disabled', false);
                      if(res.status=='1'){ // Success
                        ...
                        $('#invoiceype_datatable').DataTable().ajax.reload(null,false);
...
function delete_invoice_type(ele,service_id){
    if(confirm("Are you sure wanted to delete this Service?")){
        ...
        jQuery.ajax({
            dataType: 'json',
            type: "POST",
            url: bases_url+"Masters/delete_invoicetype",
            data: {'jsonObj' : q},
            cache: false,
            beforeSend: function(){ 
...
function validate_update_invoice_type(ele){
	...
	if(jQuery.trim(service_name)==''){
		 showError("Please Enter Invoice Type","service_name"); hasError = 1; 
	}else{
	...
	if(hasError==1){
		return false;
			...
			jQuery.ajax({
				dataType: 'json',
				type: "POST",
				url: bases_url+"Masters/Update_Invoicetype",
				data: {'jsonObj' : q},
				cache: false,
				beforeSend: function(){ 
...
$(document).on('click', '.a_category_view', function () {
    var id = $(this).attr("id");
       $.ajax({
          url: bases_url+"Masters/get_all_invoicetype",
          method: "POST",
          data: {
             id: id
          },
